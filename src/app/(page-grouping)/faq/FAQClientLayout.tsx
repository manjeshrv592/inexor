            ...
            <div className="relative xl:h-full">
                <div className="absolute inset-0 size-full">
                    ...
                    <img
                        ...
                        className={`h-full w-full object-cover ${faqPageSettings?.sidebarImage?.isGrayscale !== false
                                ? "grayscale"
                                : ""
                            }`}
                        loading="eager"
...
interface FAQClientLayoutProps {
    categories: FAQCategory[];
    allFaqItems: FAQItemType[];
    faqPageData: FAQPageType | null;
    faqPageSettings: FAQPageSettings | null;
    children: React.ReactNode;
}

const FAQClientLayout = ({
    categories,
    allFaqItems,
    faqPageData,
    faqPageSettings,
    ...
    // Determine selected category index from URL
    const selectedCategoryIndex = useMemo(() => {
        if (!currentCategorySlug || categories.length === 0) return 0;
    ...
    // Filter FAQ items for current category (from all items already in memory)
    const faqItems = useMemo(() => {
        ...
        return allFaqItems
            .filter((item) => item.category?.slug?.current === currentCategory.slug.current)
            .sort((a, b) => (a.order || 0) - (b.order || 0));
    }, [allFaqItems, categories, selectedCategoryIndex]);

    // Determine active question from URL or default to first
    const activeQuestionId = useMemo(() => {
        if (currentQuestionSlug && faqItems.length > 0) {
            const targetQuestion = faqItems.find(
                (item) => item.slug.current === currentQuestionSlug
            );
            if (targetQuestion) {
                return targetQuestion._id;
            }
        }
        // Default to first question in category
        return faqItems.length > 0 ? faqItems[0]._id : null;
    }, [currentQuestionSlug, faqItems]);
    ...
    return (
        ...
        >
            ...
            <MiddlePanel faqPageData={faqPageData} activeQuestion={activeQuestion} />
